...
def prepare_dataset(sentences, targets, word_to_id, char_to_id, tag_to_id, dtype, lower=False):
    ...
    for index, s in enumerate(sentences):
        ...
        str_words = s[0].split()
        words = []
        # print("string_words", str_words)
        # chars = [[char_to_id[c] for c in w.lower()] for w in str_words]
        chars = []
        for w in str_words:
            # print("str_words_eval", str_words)
...
def prediction(model, datas, data_model, predict_file, dataset="Train"):
    # Initializations
    ...
    # model = Parallel_LSTM_CRF()
    model.load_state_dict(torch.load('./models/hindi_1', map_location = torch.device('cpu')))
    test_sentence = "गिरजा आज फिर उस औरत को साथ लाया था.वही दुबली पतली मोटी-मोटी आंखें तीखी नाक और सांवले रंग वाली औरत।"
    ...
    for sentence in f.readlines():
        ...
        chars = []
        data = {}
        targets = data_model.train_targets
        word_to_id = data_model.word2index
        char_to_id = data_model.char2index
        tag_to_id = data_model.tag2index
        ...
        for w in str_words:
            try:
                ...
                pass
        tags = [tag_to_id[tag] for tag in targets[0].split(" ")]

        data['str_words'] = str_words
        data['words'] = words
        data['chars'] = chars
        data['tags'] = tags
        # data.append({
    ...
    # print(data[0]['str_words'])
        ...
        print("Predicted Chunks :", lab_pred_chunks)
        print("\n")
    return
...
def main(argv):
    ...
    print("tag2index",data_model.tag2index)
    ...
    prediction(model, train_data, data_model, predict_file)
    exit(0)
